...
Скрапер проверяет расписание каждые 30–60 секунд. Уведомление отправляется **только при смене состояния**: когда для спектакля не было доступных билетов, а теперь они появились. Если билеты есть уже несколько итераций подряд — повторные уведомления не отправляются.

Состояние доступности хранится в колонке `tickets_available` таблицы `performances`. При исчезновении билетов флаг сбрасывается — и при следующем их появлении уведомление придёт снова.

Незакрытые уведомления (webhook-пуш не удался) повторно отправляются на каждой итерации до успешной доставки.
